...
const Misson = ({ match, address, tokenBalance }) => {
  ...
  const fetchMission = async () => { //현재 미션 정보 읽어오기
    try {
  ...
  const onLike =()=>{
   ...
   const checkMission = (array, value) => { //내가 참여했는지 확인하는 함수
     console.log(array)
     return array.some(arrayValue => value === arrayValue);
   }

   const addBlock = () => { //블록에 기록하는 함수
    const from = address;
    contract.methods
  ...
  const addDatabase = () => { //데이터베이스에 기록하는 함수(내가 참여한 기록 추가)
      const obj = {
       account: setAddress.address,
       missionId: missionId,
     };
     console.log(obj)
     axios.post("http://localhost:5000/api/accounts/addMission", obj).then(
  ...
  const updateMission = () => { //데이터베이스에 기록하는 함수(현재 미션에 내 지갑 주소 추가, 좋아요 +1)
    const obj = {
      missionId: missionId,
      account: setAddress.address,
   };
  ...
  return(
    ...
    <section className={styles.details}>
      ...
      <div className={styles.datas}>
        ...
        <button className={styles.button} onClick ={onLike} type="submit">광고 보러 가기</button>
      </div>
    </section>
    <section>
